...
CREATE TABLE IF NOT EXISTS prospects (
  ...
  CONSTRAINT valid_birth_date CHECK (date_of_birth >= '1950-01-01' AND date_of_birth <= CURRENT_DATE)
);
...
CREATE TABLE IF NOT EXISTS performance_stats (
  ...
  -- Calculated Hitting Metrics (stored as regular columns)
  batting_average numeric(4,3),
  on_base_percentage numeric(4,3),
  slugging_percentage numeric(4,3),
  ...
  -- Calculated Pitching Metrics (stored as regular columns)
  earned_run_average numeric(4,2),
  whip numeric(4,3),
  strikeouts_per_nine numeric(4,1),
  walks_per_nine numeric(4,1),
...
-- Function to calculate hitting metrics
CREATE OR REPLACE FUNCTION calculate_hitting_metrics()
RETURNS TRIGGER AS $$
BEGIN
  -- Calculate batting average
  IF NEW.at_bats > 0 THEN
    NEW.batting_average = ROUND(NEW.hits::numeric / NEW.at_bats::numeric, 3);
  ELSE
    NEW.batting_average = 0;
  END IF;
  
  -- Calculate on-base percentage
  IF (NEW.at_bats + NEW.walks + NEW.hit_by_pitch + NEW.sacrifice_flies) > 0 THEN
    NEW.on_base_percentage = ROUND(
      (NEW.hits + NEW.walks + NEW.hit_by_pitch)::numeric / 
      (NEW.at_bats + NEW.walks + NEW.hit_by_pitch + NEW.sacrifice_flies)::numeric, 3
    );
  ELSE
    NEW.on_base_percentage = 0;
  END IF;
  
  -- Calculate slugging percentage
  IF NEW.at_bats > 0 THEN
    NEW.slugging_percentage = ROUND(
      (NEW.hits + NEW.doubles + (NEW.triples * 2) + (NEW.home_runs * 3))::numeric / 
      NEW.at_bats::numeric, 3
    );
  ELSE
    NEW.slugging_percentage = 0;
  END IF;
...
-- Function to calculate pitching metrics
CREATE OR REPLACE FUNCTION calculate_pitching_metrics()
RETURNS TRIGGER AS $$
BEGIN
  -- Calculate ERA
  IF NEW.innings_pitched > 0 THEN
    NEW.earned_run_average = ROUND((NEW.earned_runs * 9.0) / NEW.innings_pitched, 2);
  ELSE
    NEW.earned_run_average = 0;
  END IF;
  
  -- Calculate WHIP
  IF NEW.innings_pitched > 0 THEN
    NEW.whip = ROUND((NEW.hits_allowed + NEW.walks_allowed)::numeric / NEW.innings_pitched, 3);
  ELSE
    NEW.whip = 0;
  END IF;
  ...
  -- Calculate K/9
  IF NEW.innings_pitched > 0 THEN
    NEW.strikeouts_per_nine = ROUND((NEW.strikeouts_pitched * 9.0) / NEW.innings_pitched, 1);
  ELSE
    NEW.strikeouts_per_nine = 0;
  END IF;
  
  -- Calculate BB/9
  IF NEW.innings_pitched > 0 THEN
    NEW.walks_per_nine = ROUND((NEW.walks_allowed * 9.0) / NEW.innings_pitched, 1);
  ELSE
    NEW.walks_per_nine = 0;
  END IF;
...
CREATE TRIGGER update_prospects_full_name BEFORE INSERT OR UPDATE ON prospects
...
CREATE TRIGGER calculate_hitting_stats BEFORE INSERT OR UPDATE ON performance_stats
  FOR EACH ROW EXECUTE FUNCTION calculate_hitting_metrics();

CREATE TRIGGER calculate_pitching_stats BEFORE INSERT OR UPDATE ON performance_stats
  FOR EACH ROW EXECUTE FUNCTION calculate_pitching_metrics();

-- Create a view for prospects with calculated age
CREATE OR REPLACE VIEW prospects_with_age AS
SELECT 
  *,
  EXTRACT(YEAR FROM AGE(date_of_birth)) AS age_years,
  ROUND(
    EXTRACT(YEAR FROM AGE(date_of_birth)) + 
    (EXTRACT(DOY FROM AGE(date_of_birth)) / 365.25), 1
  ) AS age_precise
FROM prospects;